...
type ProcessHost struct {
	...
	HostId    int `json:"host_id" xorm:"int not null index"`
}
...
func (ph *ProcessHost) GetByProcess(process Process) []Host {
	hosts := make([]Host, 0)
	_ = Db.Table(ProcessHost{}).Select("`host`.*").Join("LEFT", "host", "host.id = process_host.host_id").
		Where("process_id = ?", process.Id).Find(&hosts)
	return hosts
}

func (ph *ProcessHost) DeleteForProcess(process Process) {
	_, _ = Db.Where("process_id = ?", process.Id).Delete(ph)
}
